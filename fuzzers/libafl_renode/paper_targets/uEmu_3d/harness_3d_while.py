...
# mach.load_repl("/home/asmita/fuzzing_bare-metal/SEFF_project_dirs/SEFF-project/LibAFL/fuzzers/libafl_renode/paper_targets/P2IM_Robot/stm32f103_fuzz.repl")
...
print("*********LoadElf********")
# mach.load_elf("uEmu.3Dprinter.elf")
mach.load_elf("uEmu.3Dprinter_patched.elf") #patched for delay but mach.sysbus.cpu.EnableTimeSkip("delay") wokrs
...
target_addr = 0x080003a0 #reset handler - named as __start__
...
fault_addr1 = mach.sysbus.GetSymbolAddress("__exc_busfault")
print(f"BusFault_Handler addr : {hex(fault_addr1)}")

fault_addr2 = mach.sysbus.GetSymbolAddress("__exc_hardfault")
print(f"UsageFault_Handler addr : {hex(fault_addr2)}")

fault_addr3 = mach.sysbus.GetSymbolAddress("__exc_usagefault")
print(f"HardFault_Handler addr : {hex(fault_addr3)}")

fault_addr4 = mach.sysbus.GetSymbolAddress("__error")
...
ret_val = 0
exit_addr = 0x0800303e  # this will change depending on target
...
# i2c_wait_addr = mach.sysbus.GetSymbolAddress("I2C_WaitOnRXNEFlagUntilTimeout")
# print(f"i2c_wait_addr : {hex(i2c_wait_addr)}")

# def hook_addr_i2c_wait(cpu,addr):
   # # mach.Pause()
#     # print(f"***** Entered hook_addr_i2c_wait ******* : {hex(addr)}")
#     mach.sysbus.i2c1.GetRXNE_Fuzz()
#     mach.sysbus.i2c1.SetRXNE_Fuzz()
#     print(f"***** hook_addr_i2c_wait ******* : {hex(addr)}")

# Action4 = getattr(System, 'Action`2')
# hook_addr_i2c_wait_act = Action4[ICpuSupportingGdb, System.UInt64](hook_addr_i2c_wait)
# mach.sysbus.cpu.AddHook(i2c_wait_addr,hook_addr_i2c_wait_act)
...
mach.ConfigurePeripheralsToReset(["cpu","nvic","flash_ctrl","timer2","timer4"])
...
# mach.sysbus.cpu.EnableTimeSkip("delay") #****needed, but slow
...
while i<10: # clibafl_renode_lib = ctypes.CDLL("liblibafl_renode.so")omment this when replaying
# while True:
    # try:
        print("^^^^^ Loop starting")
        i+=1
        ...
        mach.FuzzReset()
        # mach.sysbus.usart2.WriteChar(0xaa) 
        mach.sysbus.usart1.ReadFromFuzzer_PY(data)
        # mach.sysbus.i2c1.ReadFromFuzzer_PY_i2c(data)
        #     
        ...
        if exit_event.wait(timeout=2):
            ...
            # mach.sysbus.cpu.Fuzz_LoadState()
            print(f"^^^^^^^^^ Error: Timeout/Crash waiting for Exit event. PC : {mach.sysbus.cpu.PC}")
