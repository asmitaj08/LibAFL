...
print("*********LoadElf********")
# mach.load_elf("uEmu.3Dprinter.elf") #patched for delay but event mach.sysbus.cpu.EnableTimeSkip("delay") works
mach.load_elf("uEmu.3Dprinter_patched.elf")
...
exit_addr = 0x0800303e  # this will change depending on target -(while-true loop)
...
# mach.sysbus.cpu.EnableTimeSkip("delay") #****needed
...
def callback():
        ...
        if exit_event.wait(timeout=2):
            ...
            ret_val = 2
            mach.Pause()
            print(f"^^^^^^^^^ Error: Timeout/crash waiting for Exit event. ret val : {ret_val}")
