...
use core::{convert::TryInto, ffi::c_void, slice, ptr::addr_of, time::Duration};
...
use libafl::{
    ...
    events::{Launcher, EventConfig, SimpleEventManager, setup_restarting_mgr_std},
...
pub extern "C" fn main_fuzzing_func(input_dir: *const c_char,
    ...
    // let mut run_client = |state: Option<_>, mut restarting_mgr, _client_description| {
        ...
        state.load_initial_inputs(&mut fuzzer, &mut executor, &mut restarting_mgr, &[PathBuf::from("/home/asmita/fuzzing_bare-metal/SEFF_project_dirs/SEFF-project/LibAFL/fuzzers/libafl_renode/input_dir/")]).unwrap();
