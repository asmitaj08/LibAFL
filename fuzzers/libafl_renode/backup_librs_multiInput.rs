...
#[no_mangle] 
static mut UART_INPUT_DATA: [u8; INPUT_SIZE_MAX] = [0; INPUT_SIZE_MAX];
// static ref INPUT_DATA: Mutex<[u8; MAP_SIZE]> = Mutex::new([0; INPUT_SIZE]);
// static mut INPUT_DATA: Vec<u8> = Vec::new();

#[no_mangle] 
pub extern "C" fn uart_get_input_ptr() -> *mut u8 { 
    unsafe{
        let ptr = UART_INPUT_DATA.as_mut_ptr();
        // let in_data = INPUT_DATA.lock().unwrap();
        // let ptr = in_data.as_mut_ptr();
        println!("*****UART_INPUT_DATA Pointer Address - Libafl: {:?}", ptr);
        ptr
...
#[no_mangle] 
static mut UART_INPUT_SIZE: usize = 0;

#[no_mangle] 
pub extern "C" fn uart_get_input_size_ptr() -> *mut usize { 
    unsafe{
        let ptr : *mut usize = &mut UART_INPUT_SIZE;
        println!("*****UART_INPUT_SIZE Pointer Address - Libafl: {:?}", ptr);
        // println!("******Coverage Map Pointer Address (pointer format): {:p}", ptr);
        ptr
...
#[no_mangle] 
static mut I2C_INPUT_DATA: [u8; INPUT_SIZE_MAX] = [0; INPUT_SIZE_MAX];
// static ref INPUT_DATA: Mutex<[u8; MAP_SIZE]> = Mutex::new([0; INPUT_SIZE]);
// static mut INPUT_DATA: Vec<u8> = Vec::new();

#[no_mangle] 
pub extern "C" fn i2c_get_input_ptr() -> *mut u8 { 
    unsafe{
        let ptr = I2C_INPUT_DATA.as_mut_ptr();
        // let in_data = INPUT_DATA.lock().unwrap();
        // let ptr = in_data.as_mut_ptr();
        println!("*****I2C_INPUT_DATA Pointer Address - Libafl: {:?}", ptr);
        ptr
...
#[no_mangle] 
static mut I2C_INPUT_SIZE: usize = 0;

#[no_mangle] 
pub extern "C" fn i2c_get_input_size_ptr() -> *mut usize { 
    unsafe{
        let ptr : *mut usize = &mut I2C_INPUT_SIZE;
        println!("*****I2C_INPUT_SIZE Pointer Address - Libafl: {:?}", ptr);
        // println!("******Coverage Map Pointer Address (pointer format): {:p}", ptr);
        ptr
...
pub extern "C" fn main_fuzzing_func(input_dir: *const c_char,
    ...
    let mut harness = |input: &MultipartInput<BytesInput, String>| {
        ...
        for (i, (name, input)) in input.parts().iter().enumerate() { 
            ...
            if name == "uart" {
                ...
                unsafe{
                    UART_INPUT_DATA[..len].copy_from_slice(&buf[..len]);
                    UART_INPUT_SIZE = len;
                }
                }
           }
                ...
                if !buf.is_empty() {
                    let len = std::cmp::min(buf.len(), INPUT_SIZE_MAX);
                    unsafe{
                        I2C_INPUT_DATA[..len].copy_from_slice(&buf[..len]);
                        I2C_INPUT_SIZE = len;
                    }
                }
            }
   ...
   let edges = unsafe { &mut COV_MAP }; //orig
    ...
    let initial = MultipartInput::from(vec![ 
        ...
        // let part1 = ValueInput::new(b"A".to_vec());
        // let part2 = ValueInput::new(b"D".to_vec());
        // let initial = MultipartInput::from(vec![part1, part2]);

        println!("Total initial Input length: {}", initial.len());
        println!("Multipart initial Input: {:?}", initial);
